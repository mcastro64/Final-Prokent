...
head(metro_fmr_by_zip_and_num_bds, n=10)
...
market_rate_by_zip_and_num_bds <- market_rates |>
  ...
  summarise(
    market = round(quantile(market_rate,probs=0.4))
  )
```
...
combined_df_by_zip_diffs <- combined_df_by_zip |>
  mutate(
    margin_of_error = market - fmr,
    at_or_below_fmr = as.factor(fmr >= market)
  ) |>
  select(-c(fmr, market))
...
ggplot(combined_df_by_zip_diffs, aes(y=fct_rev(bedrooms), fill=at_or_below_fmr)) +
  ...
  labs(
    title = "Count of Apartments by Size",
    x = "# of Apartments",
...
```{r }
ggplot(combined_df_by_zip_and_type, aes(rent, fct_rev(bedrooms), fill=fct_rev(type))) +
