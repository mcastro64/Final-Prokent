...
combined_market_rates <- market_rates  |>
  left_join(metro_fmr_by_zip_and_num_bds, by= c('zip_code'='zip_code', 'bedrooms'='bedrooms')) |>
  mutate(
...
# C8: Dollar differences
ggplot(combined_market_rates, aes(abs(margin_of_error), fct_rev(bedrooms), fill=at_or_below_fmr)) +
  geom_boxplot(color="#666666") +
  theme(
    legend.position="top", 
    plot.title = element_text(hjust = 0.5)
...
```{r calc-msa-avg-market-rent, warning=FALSE}
