...
Trulia will block the parser if too many requests come in a short period of time from the same IP address. Once way to get around this was to implement a delayed query
...
market_rate_by_zip_and_num_bds <- market_rates |>
  ...
  summarise(
    market = round(mean(market_rate)),
  )
```


### C2: Combine FMR and Market Rate Dataframes By Zip Code
...
combined_df_by_zip_diffs <- combined_df_by_zip |>
  mutate(
    cost_difference = market - fmr,
    at_or_below_fmr = fmr >= market
  ) |>
  select(-c(fmr, market))

combined_df_by_zip_long <- combined_df_by_zip |>
  pivot_longer(
    cols = c(fmr, market),
    names_to = c("type"),
    values_to = "rent"
...
glimpse(combined_df_by_zip)
count_diffs <- combined_df_by_zip_diffs |>
  group_by(across(all_of(c("cbsa_code", "bedrooms", "at_or_below_fmr")))) |>
  summarise(
    count = n()
  )
...
ggplot(combined_df_by_zip_diffs, aes(y=bedrooms, fill=at_or_below_fmr)) +
  geom_bar(position = position_dodge()) +
  facet_wrap(~cbsa_code, ncol=1)
...
ggplot(combined_df_by_zip, aes(cost_difference)) +
  geom_histogram() +
  facet_grid(~cbsa_code, ncol=1) 
...
ggplot(combined_df_by_zip_diffs, aes(cost_difference)) +
  geom_histogram() +
  facet_grid(vars(cbsa_code), vars(bedrooms)) 
...
```for (i in 1:nrow(metro_areas)) {
  ...
  df <- combined_df_by_zip_long |>
    filter(cbsa_code == metro_areas[i,]$cbsa_code)
  
  rents_v_fmr <- ggplot(df, aes(x=zip_code, y=rent, color=type)) +
    geom_point() +
    facet_wrap(~bedrooms, ncol=1)
...
market_rate_msa_by_num_bds <- market_rates |>
  mutate(
    cbsa_code = case_when(
      as.integer(zip_code) > 90000 ~  'METRO41740M41740',
      as.integer(zip_code) < 20000 ~ 'METRO15380M15380',
      TRUE ~ 'METRO12060M12060'
    )
  ) |>
  group_by(across(all_of(c("cbsa_code", "bedrooms")))) |>
  summarise(
    avg_market_rate_rent = mean(market_rate)
  )

# join fmr and market rate dataframes at the MSA level
combined_df_by_msa <- metro_fmr_msa_by_num_bds |>
  right_join(market_rate_msa_by_num_bds, by=c("cbsa_code", "bedrooms"))

head(combined_df_by_msa)
